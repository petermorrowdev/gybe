...
class HPAScalingRules(BaseModel):
    ...
    stabilizationWindowSeconds: Optional[int] = Field(
        ...
        description=(
            ...
            " For scale up: 0 (i.e. no stabilization is done). - For scale down: 300"
...
class MetricSpec(BaseModel):
    containerResource: Optional[ContainerResourceMetricSource] = Field(
        ...
        description=(
            ...
            " each pod of the current scale target (e.g. CPU or memory). Such metrics"
            " are built in to Kubernetes, and have special scaling options on top of"
            ' those available to normal per-pod metrics using the "pods" source. This'
            " is an alpha feature and can be enabled by the HPAContainerMetrics feature"
            " flag."
        ),
    )
    ...
    type: str = Field(
        ...
        description=(
            ...
            ' mapping to a matching field in the object. Note: "ContainerResource" type'
...
class MetricStatus(BaseModel):
    containerResource: Optional[ContainerResourceMetricStatus] = Field(
        ...
        description=(
            ...
            " requests and limits) known to Kubernetes describing a single container in"
            " each pod in the current scale target (e.g. CPU or memory). Such metrics"
            " are built in to Kubernetes, and have special scaling options on top of"
    ...
    type: str = Field(
        ...
        description=(
            ...
            ' "External", "Object", "Pods" or "Resource", each corresponds to a'
            ' matching field in the object. Note: "ContainerResource" type is available'
            " on when the feature-gate HPAContainerMetrics is enabled"
        ),
    )
