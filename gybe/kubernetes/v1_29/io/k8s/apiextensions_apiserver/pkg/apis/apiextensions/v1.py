...
class CustomResourceSubresourceScale(BaseModel):
    labelSelectorPath: Optional[str] = Field(
        ...
        description=(
            ...
            " notation are allowed. Must be a JSON Path under `.status` or `.spec`."
            " Must be set to work with HorizontalPodAutoscaler. The field pointed by"
            " this JSON path must be a string field (not a complex selector struct)"
    ...
    specReplicasPath: str = Field(
        ...
        description=(
            ...
            " value under the given path in the custom resource, the `/scale`"
...
class JSON(BaseModel):
    __root__: Any = Field(
        ...
        description=(
            ...
            " int64, float64, string, []interfaceNone, map[string]interface{} and nil."
        ),
    )
...
class ServiceReference(BaseModel):
    ...
    port: Optional[int] = Field(
        ...
        description=(
            ...
            " `port` should be a valid port number (1-65535, inclusive). Defaults to"
...
class ValidationRule(BaseModel):
    fieldPath: Optional[str] = Field(
        None,
        description=(
            "fieldPath represents the field path returned when the validation fails. It"
            " must be a relative JSON path (i.e. with array notation) scoped to the"
    ...
    messageExpression: Optional[str] = Field(
        ...
        description=(
            ...
            " string. If both message and messageExpression are present on a rule, then"
            " messageExpression will be used if validation fails. If messageExpression"
            " results in a runtime error, the runtime error is logged, and the"
            " validation failure message is produced as if the messageExpression field"
            " were unset. If messageExpression evaluates to an empty string, a string"
            " with only spaces, or a string that contains line breaks, then the"
            " validation failure message will also be produced as if the"
            " messageExpression field were unset, and the fact that messageExpression"
            " produced an empty string/string with only spaces/string with line breaks"
            " will be logged. messageExpression has access to all the same variables as"
            ' the rule; the only difference is the return type. Example: "x must be'
            ' less than max ("+string(self.max)+")"'
        ),
    )
    optionalOldSelf: Optional[bool] = Field(
        None,
        description=(
            "optionalOldSelf is used to opt a transition rule into evaluation even when"
            " the object is first created, or if the old object is missing the"
            " value.\n\nWhen enabled `oldSelf` will be a CEL optional whose value will"
            " be `None` if there is no old value, or when the object is initially"
            " created.\n\nYou may check for presence of oldSelf using"
            " `oldSelf.hasValue()` and unwrap it after checking using"
            " `oldSelf.value()`. Check the CEL documentation for Optional types for"
            " more information:"
    ...
    reason: Optional[str] = Field(
        ...
        description=(
            ...
            ' "FieldValueInvalid". All future added reasons must be accepted by clients'
            " when reading this value and unknown reasons should be treated as"
            " FieldValueInvalid."
        ),
    )
    rule: str = Field(
        ...
        description=(
            ...
            " in the CEL expression is bound to the scoped value. Example: - Rule"
            ' scoped to the root of a resource with a status subresource: {"rule":'
            ' "self.status.actual <= self.spec.maxDesired"}\n\nIf the Rule is scoped to'
            " an object with properties, the accessible properties of the object are"
            " field selectable via `self.field` and field presence can be checked via"
            " `has(self.field)`. Null valued fields are treated as absent fields in CEL"
            " expressions. If the Rule is scoped to an object with additionalProperties"
            " (i.e. a map) the value of the map are accessible via `self[mapKey]`, map"
            " containment can be checked via `mapKey in self` and all entries of the"
            " map are accessible via CEL macros and functions such as `self.all(...)`."
            " If the Rule is scoped to an array, the elements of the array are"
            " accessible via `self[i]` and also by macros and functions. If the Rule is"
            " scoped to a scalar, `self` is bound to the scalar value. Examples: - Rule"
            ' scoped to a map of objects: {"rule":'
            ...
            " and `metadata.generateName` are always accessible from the root of the"
            " object and from any x-kubernetes-embedded-resource annotated objects. No"
            " other metadata properties are accessible.\n\nUnknown data preserved in"
            " custom resources via x-kubernetes-preserve-unknown-fields is not"
            " accessible in CEL expressions. This includes: - Unknown field values that"
            " are preserved by object schemas with"
            " x-kubernetes-preserve-unknown-fields. - Object properties where the"
            ...
            ' items schema is of an "unknown type"\n  - An object where the'
            ' additionalProperties schema is of an "unknown type"\n\nOnly property'
            " names of the form `[a-zA-Z_.-/][a-zA-Z0-9_.-/]*` are accessible."
            " Accessible property names are escaped according to the following rules"
            " when accessed in the expression: - '__' escapes to '__underscores__'"
            ...
            ' "namespace", "return".\nExamples:\n  - Rule accessing a property named'
            ' "namespace": {"rule": "self.__namespace__ > 0"}\n  - Rule accessing a'
            ' property named "x-prop": {"rule": "self.x__dash__prop > 0"}\n  - Rule'
            ' accessing a property named "redact__d": {"rule":'
            ' "self.redact__underscores__d > 0"}\n\nEquality on arrays with'
            " x-kubernetes-list-type of 'set' or 'map' ignores element order, i.e."
            " [1, 2] == [2, 1]. Concatenation on arrays with x-kubernetes-list-type use"
            " the semantics of the list type:\n  - 'set': `X + Y` performs a union"
            " where the array positions of all elements in `X` are preserved and\n   "
            ...
            " a `transition rule`.\n\nBy default, the `oldSelf` variable is the same"
            " type as `self`. When `optionalOldSelf` is true, the `oldSelf` variable is"
            " a CEL optional\n variable whose value() is the same type as `self`.\nSee"
            " the documentation for the `optionalOldSelf` field for"
            " details.\n\nTransition rules by default are applied only on UPDATE"
            " requests and are skipped if an old value could not be found. You can opt"
            " a transition rule into unconditional evaluation by setting"
            " `optionalOldSelf` to true."
        ),
    )
...
class WebhookClientConfig(BaseModel):
    ...
    service: Optional[ServiceReference] = Field(
        ...
        description=(
            ...
            " url must be specified.\n\nIf the webhook is running within the cluster,"
...
class CustomResourceDefinitionStatus(BaseModel):
    ...
    storedVersions: Optional[List[str]] = Field(
        ...
        description=(
            ...
            " Versions may not be removed from `spec.versions` while they exist in this"
            " list."
        ),
    )
...
class JSONSchemaProps(BaseModel):
    ...
    format: Optional[str] = Field(
        ...
        description=(
            ...
            " i.e. a 24 characters hex string - uri: an URI as parsed by Golang"
            " net/url.ParseRequestURI - email: an email address as parsed by Golang"
            " net/mail.ParseAddress - hostname: a valid representation for an Internet"
            " host name, as defined by RFC 1034, section 3.1 [RFC1034]. - ipv4: an IPv4"
            " IP as parsed by Golang net.ParseIP - ipv6: an IPv6 IP as parsed by Golang"
            " net.ParseIP - cidr: a CIDR as parsed by Golang net.ParseCIDR - mac: a MAC"
            " address as parsed by Golang net.ParseMAC - uuid: an UUID that allows"
    ...
    x_kubernetes_int_or_string: Optional[bool] = Field(
        ...
        description=(
            "x-kubernetes-int-or-string specifies that this value is either an integer"
            " or a string. If this is true, an empty type is allowed and type as child"
            " of anyOf is permitted if following one of the following patterns:\n\n1)"
            " anyOf:\n   - type: integer\n   - type: string\n2) allOf:\n   - anyOf:\n  "
            "   - type: integer\n     - type: string\n   - ... zero or more"
        ),
    )
    x_kubernetes_list_map_keys: Optional[List[str]] = Field(
        ...
        description=(
            ...
            " structure (no nesting is supported).\n\nThe properties specified must"
            " either be required or have a default value, to ensure those properties"
    ...
    x_kubernetes_list_type: Optional[str] = Field(
        ...
        description=(
            ...
            " with the same value. Each\n     value must be a scalar, an object with"
            " x-kubernetes-map-type `atomic` or an\n     array with"
            " x-kubernetes-list-type `atomic`.\n3) `map`:\n     These lists are like"
            " maps in that their elements have a non-index key\n     used to identify"
            " them. Order is preserved upon merge. The map tag\n     must only be used"
            " on a list with elements of type object.\nDefaults to atomic for arrays."
        ),
    )
    x_kubernetes_map_type: Optional[str] = Field(
        ...
        description=(
            ...
            " have 2 possible values:\n\n1) `granular`:\n     These maps are actual"
            " maps (key-value pairs) and each fields are independent\n     from each"
            " other (they can each be manipulated by separate actors). This is\n    "
            " the default behaviour for all maps.\n2) `atomic`: the list is treated as"
            " a single entity, like a scalar.\n     Atomic maps will be entirely"
...
class CustomResourceDefinitionVersion(BaseModel):
    ...
    deprecationWarning: Optional[str] = Field(
        ...
        description=(
            ...
            " this version is deprecated and recommends use of the newest served"
    ...
    name: str = Field(
        ...,
        description=(
            "name is the version name, e.g. “v1”, “v2beta1”, etc. The custom resources"
            " are served under this version at `/apis/<group>/<version>/...` if"
            " `served` is true."
        ),
    )
...
class CustomResourceDefinitionSpec(BaseModel):
    ...
    versions: List[CustomResourceDefinitionVersion] = Field(
        ...
        description=(
            ...
            ' listed in API discovery. If the version string is "kube-like", it will'
