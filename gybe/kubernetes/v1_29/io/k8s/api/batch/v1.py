...
class PodFailurePolicyRule(BaseModel):
    action: str = Field(
        ...
        description=(
            ...
            " indicates that the pod is handled in the default way - the\n  counter"
            " towards the .backoffLimit is incremented.\nAdditional values are"
            " considered to be added in the future. Clients should react to an unknown"
...
class JobStatus(BaseModel):
    ...
    failedIndexes: Optional[str] = Field(
        None,
        description=(
            ...
            " `completedIndexes` field, ie. they are kept as decimal integers separated"
            " by commas. The numbers are listed in increasing order. Three or more"
            " consecutive numbers are compressed and represented by the first and last"
            " element of the series, separated by a hyphen. For example, if the failed"
            ' indexes are 1, 3, 4, 5 and 7, they are represented as "1,3-5,7". This'
            " field is beta-level. It can be used when the `JobBackoffLimitPerIndex`"
            " feature gate is enabled (enabled by default)."
        ),
    )
    ready: Optional[int] = Field(
        None, description="The number of pods which have a Ready condition."
    )
    ...
    terminating: Optional[int] = Field(
        None,
        description=(
            "The number of pods which are terminating (in phase Pending or Running and"
            " have a deletionTimestamp).\n\nThis field is beta-level. The job"
            " controller populates the field when the feature gate"
...
class JobSpec(BaseModel):
    ...
    backoffLimitPerIndex: Optional[int] = Field(
        None,
        description=(
            "Specifies the limit for the number of retries within an index before"
            " marking this index as failed. When enabled the number of failures per"
            " index is kept in the pod's batch.kubernetes.io/job-index-failure-count"
            " annotation. It can only be set when Job's completionMode=Indexed, and the"
            " Pod's restart policy is Never. The field is immutable. This field is"
    ...
    completionMode: Optional[str] = Field(
        ...
        description=(
            ...
            " doesn't recognize, which is possible during upgrades due to version skew,"
    ...
    maxFailedIndexes: Optional[int] = Field(
        None,
        description=(
            ...
            " all of its indexes and is marked with the `Complete` Job condition. It"
            " can only be specified when backoffLimitPerIndex is set. It can be null or"
            " up to completions. It is required and must be less than or equal to 10^4"
            " when is completions greater than 10^5. This field is beta-level. It can"
            " be used when the `JobBackoffLimitPerIndex` feature gate is enabled"
            " (enabled by default)."
        ),
    )
    ...
    podReplacementPolicy: Optional[str] = Field(
        None,
        description=(
            "podReplacementPolicy specifies when to create replacement Pods. Possible"
            " values are: - TerminatingOrFailed means that we recreate pods\n  when"
            " they are terminating (has a metadata.deletionTimestamp) or failed.\n-"
            " Failed means to wait until a previously created Pod is fully terminated"
            " (has phase\n  Failed or Succeeded) before creating a replacement"
            " Pod.\n\nWhen using podFailurePolicy, Failed is the the only allowed"
            " value. TerminatingOrFailed and Failed are allowed values when"
            " podFailurePolicy is not in use. This is an beta field. To use this,"
            " enable the JobPodReplacementPolicy feature toggle. This is on by default."
        ),
    )
