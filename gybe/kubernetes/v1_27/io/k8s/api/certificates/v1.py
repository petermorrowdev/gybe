...
class CertificateSigningRequestSpec(BaseModel):
    expirationSeconds: Optional[int] = Field(
        ...
        description=(
            ...
            " the actual duration.\n\nThe v1.22+ in-tree implementations of the"
            " well-known Kubernetes signers will honor this field as long as the"
            " requested duration is not greater than the maximum duration they will"
            " honor per the --cluster-signing-duration CLI flag to the Kubernetes"
            " controller manager.\n\nCertificate signers may not honor this field for"
            " various reasons:\n\n  1. Old signer that is unaware of the field (such as"
            " the in-tree\n     implementations prior to v1.22)\n  2. Signer whose"
            " configured maximum is shorter than the requested duration\n  3. Signer"
            " whose configured minimum is longer than the requested duration\n\nThe"
            " minimum valid value for expirationSeconds is 600, i.e. 10 minutes."
        ),
    )
    ...
    signerName: str = Field(
        ...,
        description=(
            "signerName indicates the requested signer, and is a qualified"
            ...
            " issues client certificates that can be used to authenticate to"
            ...
            ' in kube-controller-manager.\n 3. "kubernetes.io/kubelet-serving" issues'
            " serving certificates that kubelets use to serve TLS endpoints, which"
            " kube-apiserver can connect to securely.\n  Requests for this signer are"
            " never auto-approved by kube-controller-manager, and can be issued by the"
            ...
            " distribution: how trust (CA bundles) are distributed.\n 2. Permitted"
            " subjects: and behavior when a disallowed subject is requested.\n 3."
            " Required, permitted, or forbidden x509 extensions in the request"
            " (including whether subjectAltNames are allowed, which types, restrictions"
            " on allowed values) and behavior when a disallowed extension is"
            " requested.\n 4. Required, permitted, or forbidden key usages / extended"
            " key usages.\n 5. Expiration/certificate lifetime: whether it is fixed by"
            " the signer, configurable by the admin.\n 6. Whether or not requests for"
            " CA certificates are allowed."
        ),
    )
    ...
    usages: Optional[List[str]] = Field(
        ...
        description=(
            ...
            ' TLS serving certificates typically request: "key encipherment", "digital'
            ' signature", "server auth".\n\nValid values are:\n "signing", "digital'
            ' signature", "content commitment",\n "key encipherment", "key agreement",'
            ' "data encipherment",\n "cert sign", "crl sign", "encipher only",'
            ' "decipher only", "any",\n "server auth", "client auth",\n "code signing",'
            ' "email protection", "s/mime",\n "ipsec end system", "ipsec tunnel",'
...
class CertificateSigningRequestCondition(BaseModel):
    ...
    type: str = Field(
        ...
        description=(
            ...
            " subresource, indicating the request was approved and should be issued by"
            ' the signer.\n\nA "Denied" condition is added via the /approval'
            " subresource, indicating the request was denied and should not be issued"
            ' by the signer.\n\nA "Failed" condition is added via the /status'
            " subresource, indicating the signer failed to issue the"
            " certificate.\n\nApproved and Denied conditions are mutually exclusive."
            " Approved, Denied, and Failed conditions cannot be removed once"
...
class CertificateSigningRequestStatus(BaseModel):
    certificate: Optional[str] = Field(
        ...
        description=(
            ...
            ' certificate signing request is denied, a condition of type "Denied" is'
            ...
            " empty.\n\nValidation requirements:\n 1. certificate must contain one or"
            ' more PEM blocks.\n 2. All PEM blocks must have the "CERTIFICATE" label,'
            " contain no headers, and the encoded data\n  must be a BER-encoded ASN.1"
            " Certificate structure as described in section 4 of RFC5280.\n 3. Non-PEM"
            ' content may appear before or after the "CERTIFICATE" PEM blocks and is'
            " unvalidated,\n  to allow for explanatory text as described in section 5.2"
            " of RFC7468.\n\nIf more than one PEM block is present, and the definition"
            " of the requested spec.signerName does not indicate otherwise, the first"
            " block is the issued certificate, and subsequent blocks should be treated"
            " as intermediate certificates and presented in TLS handshakes.\n\nThe"
            " certificate is encoded in PEM format.\n\nWhen serialized as JSON or YAML,"
            " the data is additionally base64-encoded, so it consists of:\n\n   "
            " base64(\n    -----BEGIN CERTIFICATE-----\n    ...\n    -----END"
            " CERTIFICATE-----\n    )"
        ),
    )
