...
class CSIPersistentVolumeSource(K8sSpec):
    ...
    ----------
        controllerExpandSecretRef: controllerExpandSecretRef is a reference to the secret object containing
            ...
            This is an beta field and requires enabling ExpandCSIVolumes feature gate. This field is optional,
        ...
        nodeExpandSecretRef: nodeExpandSecretRef is a reference to the secret object containing sensitive
            information to pass to the CSI driver to complete the CSI NodeExpandVolume call. This is an alpha
            field and requires enabling CSINodeExpandSecret feature gate. This field is optional, may be
            omitted if no secret is required. If the secret object contains more than one secret, all secrets
            are passed.
...
class ClaimSource(K8sSpec):
    ...
    ----------
        ...
        resourceClaimTemplateName: ResourceClaimTemplateName is the name of a ResourceClaimTemplate object in
            ...
            name of the ResourceClaim will be <pod name>-<resource name>, where <resource name> is the
            PodResourceClaim.Name. Pod validation will reject the pod if the concatenated name is not valid
            for a ResourceClaim (e.g. too long).  An existing ResourceClaim with that name that is not owned
            by the pod will not be used for the pod to avoid using an unrelated resource by mistake.
            Scheduling and pod startup are then blocked until the unrelated ResourceClaim is removed.  This
            field is immutable and no changes will be made to the corresponding ResourceClaim by the control
...
class EmptyDirVolumeSource(K8sSpec):
    ...
    sizeLimit: Optional[gybe.k8s.v1_26.api.resource.Quantity] = None
...
class PersistentVolumeClaimSpec(K8sSpec):
    ...
    resources: Optional[ResourceRequirements] = None
    selector: Optional[gybe.k8s.v1_26.meta.v1.LabelSelector] = None
    storageClassName: Optional[str] = None
    volumeMode: Optional[str] = None
    volumeName: Optional[str] = None
...
class PersistentVolumeClaimStatus(K8sSpec):
    ...
    ----------
        ...
        allocatedResources: allocatedResources is the storage resource within AllocatedResources tracks the
            capacity allocated to a PVC. It may be larger than the actual capacity when a volume expansion
            ...
            equal or lower than the requested capacity. This is an alpha field and requires enabling
        ...
        resizeStatus: resizeStatus stores status of resize operation. ResizeStatus is not set by default but
            when expansion is complete resizeStatus is set to empty string by resize controller or kubelet.
...
class PodAffinityTerm(K8sSpec):
    ...
    topologyKey: str
    labelSelector: Optional[gybe.k8s.v1_26.meta.v1.LabelSelector] = None
...
class PodSecurityContext(K8sSpec):
    ...
    ----------
        ...
        supplementalGroups: A list of groups applied to the first process run in each container, in addition
            to the container's primary GID, the fsGroup (if specified), and group memberships defined in the
...
class PodSpec(K8sSpec):
    ...
    ----------
        ...
        nodeName: NodeName is a request to schedule this pod onto a specific node. If it is non-empty, the
        ...
        restartPolicy: Restart policy for all containers within the pod. One of Always, OnFailure, Never.
            Default to Always.
...
class Probe(K8sSpec):
    ...
    ----------
        ...
        grpc: GRPC specifies an action involving a GRPC port. This is a beta field and requires enabling
...
class ResourceFieldSelector(K8sSpec):
    ...
    divisor: Optional[gybe.k8s.v1_26.api.resource.Quantity] = None
...
class SeccompProfile(K8sSpec):
    ...
    ----------
        localhostProfile: localhostProfile indicates a profile defined in a file on the node should be used.
            ...
            kubelet's configured seccomp profile location. Must only be set if type is 'Localhost'.
...
class SecurityContext(K8sSpec):
    ...
    ----------
        ...
        privileged: Run container in privileged mode. Processes in privileged containers are essentially
            ...
            spec.os.name is windows.
        procMount: procMount denotes the type of proc mount to use for the containers. The default is
...
class TopologySpreadConstraint(K8sSpec):
    ...
    ----------
        ...
        matchLabelKeys: MatchLabelKeys is a set of pod label keys to select the pods over which spreading will
            ...
            be calculated for the incoming pod. Keys that don't exist in the incoming pod labels will be
...
class WindowsSecurityContextOptions(K8sSpec):
    ...
    ----------
        ...
        hostProcess: HostProcess determines if a container should be run as a 'Host Process' container. This
            field is alpha-level and will only be honored by components that enable the
            WindowsHostProcessContainers feature flag. Setting this field without the feature flag will result
            in errors when validating the Pod. All of a Pod's containers must have the same effective
            HostProcess value (it is not allowed to have a mix of HostProcess containers and non-HostProcess
...
class ContainerStatus(K8sSpec):
    ...
    ----------
        ...
        started: Specifies whether the container has passed its startup probe. Initialized as false, becomes
            true after startupProbe is considered successful. Resets to false when the container is restarted,
...
class EndpointAddress(K8sSpec):
    ...
    ----------
        ...
        ip: The IP of this endpoint. May not be loopback (127.0.0.0/8), link-local (169.254.0.0/16), or link-
            local multicast ((224.0.0.0/24). IPv6 is also accepted but not fully supported on all platforms.
...
class Event(K8sSpec):
    ...
    metadata: gybe.k8s.v1_26.meta.v1.ObjectMeta
...
@dataclass
class PodIP(K8sSpec):
    """IP address information for entries in the (plural) PodIPs field. Each entry includes:          IP: An
...
class PodStatus(K8sSpec):
    ...
    ----------
        ...
        podIP: IP address allocated to the pod. Routable at least within the cluster. Empty if not yet
            allocated.
...
class ReplicationControllerSpec(K8sSpec):
    ...
    ----------
        ...
        template: Template is the object that describes the pod that will be created if insufficient replicas
            are detected. This takes precedence over a TemplateRef.
...
class ServiceStatus(K8sSpec):
    ...
    conditions: Optional[List[gybe.k8s.v1_26.meta.v1.Condition]] = None
