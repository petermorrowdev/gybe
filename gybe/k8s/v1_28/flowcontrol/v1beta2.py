...
@dataclass
class ExemptPriorityLevelConfiguration(K8sResource):
...
@dataclass
class FlowDistinguisherMethod(K8sResource):
...
class FlowSchema(K8sSpec):
    ...
    apiVersion: Literal['flowcontrol.apiserver.k8s.io/v1beta2'] = 'flowcontrol.apiserver.k8s.io/v1beta2'
...
@dataclass
class FlowSchemaCondition(K8sResource):
...
@dataclass
class FlowSchemaSpec(K8sResource):
...
@dataclass
class FlowSchemaStatus(K8sResource):
...
@dataclass
class GroupSubject(K8sResource):
...
@dataclass
class LimitResponse(K8sResource):
...
@dataclass
class LimitedPriorityLevelConfiguration(K8sResource):
    ...
    Attributes:
        assuredConcurrencyShares: `assuredConcurrencyShares` (ACS) configures the execution limit, which is a
            limit on the number of requests of this priority level that may be exeucting at a given time.  ACS
            must be a positive number. The server's concurrency limit (SCL) is divided among the concurrency-
            controlled priority levels in proportion to their assured concurrency shares. This produces the
            assured concurrency value (ACV) --- the number of requests that may be executing at a time --- for
            each such priority level:              ACV(l) = ceil( SCL * ACS(l) / ( sum[priority levels k]
            ACS(k) ) )  bigger numbers of ACS mean more reserved concurrent requests (at the expense of every
...
@dataclass
class NonResourcePolicyRule(K8sResource):
...
@dataclass
class PolicyRulesWithSubjects(K8sResource):
...
@dataclass
class PriorityLevelConfigurationCondition(K8sResource):
...
@dataclass
class PriorityLevelConfigurationReference(K8sResource):
...
@dataclass
class PriorityLevelConfigurationSpec(K8sResource):
...
@dataclass
class PriorityLevelConfigurationStatus(K8sResource):
...
@dataclass
class QueuingConfiguration(K8sResource):
...
@dataclass
class ResourcePolicyRule(K8sResource):
...
@dataclass
class ServiceAccountSubject(K8sResource):
