...
class IngressBackend(K8sSpec):
    ...
    resource: Optional[gybe.k8s.v1_34.core.v1.TypedLocalObjectReference] = None
...
class NetworkPolicyPeer(K8sSpec):
    ...
    podSelector: Optional[gybe.k8s.v1_34.meta.v1.LabelSelector] = None
...
class NetworkPolicySpec(K8sSpec):
    ...
    Attributes:
        ...
        podSelector: podSelector selects the pods to which this NetworkPolicy object applies. The array of
            rules is applied to any pods selected by this field. An empty selector matches all pods in the
            policy's namespace. Multiple network policies can select the same set of pods. In this case, the
            ingress rules for each are combined additively. This field is optional. If it is not specified, it
