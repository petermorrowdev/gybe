...
class AllocationResult(K8sSpec):
    ...
    nodeSelector: Optional[gybe.k8s.v1_33.core.v1.NodeSelector] = None
...
class BasicDevice(K8sSpec):
    ...
    Attributes:
        ...
        nodeName: NodeName identifies the node where the device is available.  Must only be set if
            Spec.PerDeviceNodeSelection is set to true. At most one of NodeName, NodeSelector and AllNodes can
            be set.
        nodeSelector: NodeSelector defines the nodes where the device is available.  Must only be set if
            Spec.PerDeviceNodeSelection is set to true. At most one of NodeName, NodeSelector and AllNodes can
            be set.
...
class Counter(K8sSpec):
    ...
    value: gybe.k8s.v1_33.api.resource.Quantity
...
class CounterSet(K8sSpec):
    ...
    Attributes:
        counters: Counters defines the counters that will be consumed by the device. The name of each counter
            must be unique in that set and must be a DNS label.  To ensure this uniqueness, capacities defined
            by the vendor must be listed without the driver name as domain prefix in their name. All others
            must be listed with their domain prefix.  The maximum number of counters is 32.
...
class DeviceCounterConsumption(K8sSpec):
    ...
    Attributes:
        ...
        counters: Counters defines the Counter that will be consumed by the device.  The maximum number
...
class DeviceRequest(K8sSpec):
    ...
    resource like a device, but can also ask for several identical devices.
    ...
    Attributes:
        adminAccess: AdminAccess indicates that this is a claim for administrative access to the device(s).
            ...
            resource allocations.  This field can only be set when deviceClassName is set and no subrequests
            are specified in the firstAvailable list.  This is an alpha field and requires enabling the
            DRAAdminAccess feature gate. Admin access is disabled if this field is unset or set to false,
        ...
        allocationMode: AllocationMode and its related fields define how devices are allocated to satisfy this
            ...
            must specify this field.  This field can only be set when deviceClassName is set and no
            subrequests are specified in the firstAvailable list.  More modes may get added in the future.
        ...
        count: Count is used only when the count mode is 'ExactCount'. Must be greater than zero. If
            AllocationMode is ExactCount and this field is not specified, the default is one.  This field can
            only be set when deviceClassName is set and no subrequests are specified in the firstAvailable
            list.
        deviceClassName: DeviceClassName references a specific DeviceClass, which can define additional
            configuration and selectors to be inherited by this request.  A class is required if no
            subrequests are specified in the firstAvailable list and no class can be set if subrequests are
            specified in the firstAvailable list. Which classes are available depends on the cluster.
        ...
        firstAvailable: FirstAvailable contains subrequests, of which exactly one will be satisfied by the
            scheduler to satisfy this request. It tries to satisfy them in the order in which they are listed
            here. So if there are two entries in the list, the scheduler will only check the second one if it
            determines that the first one cannot be used.  This field may only be set in the entries of
            DeviceClaim.Requests.  DRA does not yet implement scoring, so the scheduler will select the first
            set of devices that satisfies all the requests in the claim. And if the requirements can be
        ...
        selectors: Selectors define criteria which must be satisfied by a specific device in order for that
            ...
            considered.  This field can only be set when deviceClassName is set and no subrequests are
        ...
        tolerations: If specified, the request's tolerations.  Tolerations for NoSchedule are required to
            ...
            evicted, the claim will get deallocated.  The maximum number of tolerations is 16.  This field can
            only be set when deviceClassName is set and no subrequests are specified in the firstAvailable
            list.  This is an alpha field and requires enabling the DRADeviceTaints feature gate.
...
class DeviceSubRequest(K8sSpec):
    ...
    Request, but doesn't expose the AdminAccess or FirstAvailable fields, as those can only be set on the
    top-level request. AdminAccess is not supported for requests with a prioritized list, and recursive
    FirstAvailable fields are not supported.
    ...
    Attributes:
        allocationMode: AllocationMode and its related fields define how devices are allocated to satisfy this
            ...
            one. Any other requests must specify this field.  More modes may get added in the future. Clients
            must refuse to handle requests with unknown modes.
        count: Count is used only when the count mode is 'ExactCount'. Must be greater than zero. If
...
class NetworkDeviceData(K8sSpec):
    ...
    Attributes:
        ...
        ips: IPs lists the network addresses assigned to the device's network interface. This can include both
            ...
            associated subnet mask. e.g.: '192.0.2.5/24' for IPv4 and '2001:db8::5/64' for IPv6.  Must not
...
class ResourceSliceSpec(K8sSpec):
    ...
    Attributes:
        ...
        sharedCounters: SharedCounters defines a list of counter sets, each of which has a name and a list of
            ...
            maximum number of SharedCounters is 32.
